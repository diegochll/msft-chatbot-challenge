...
class GoodNews extends ComponentDialog {
    constructor(id) {
        ...
        let response = reddit.api.get('/r/Coronavirus/search?q=flair_name%3A"Good%20News"&restrict_sr=1&t=day&sort=top', {})
    ...
    async showCard(response, stepContext) {
        ...
        let shuffled = data.sort(function(){return .5 - Math.random()});
        let topNews = shuffled.slice(0,3);

        await stepContext.context.sendActivity({ attachments: [this.createAdaptiveCard(topNews,0)] });
        await stepContext.context.sendActivity({ attachments: [this.createAdaptiveCard(topNews,1)] });
        await stepContext.context.sendActivity({ attachments: [this.createAdaptiveCard(topNews,2)] });
        await stepContext.context.sendActivity("Hope this 3 Good News can cheer you up a bit! :)");
